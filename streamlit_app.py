...
def _synthetic_demo() -> pd.DataFrame:
    rows = [
        {"person_id": "SYN_001", "full_name": "Alex Morgan", "org_search_target": "Northstar Mobility", "primary_fair": "Synthetic Fair A", "preliminary_classification": ""},
        {"person_id": "SYN_002", "full_name": "Jordan Lee", "org_search_target": "Helio Systems", "primary_fair": "Synthetic Fair A", "preliminary_classification": ""},
        {"person_id": "SYN_003", "full_name": "Sam Rivera", "org_search_target": "Arcadia Labs", "primary_fair": "Synthetic Fair B", "preliminary_classification": ""},
    ]
    sources = {
        "SYN_001": [RetrievedSource(query='"Alex Morgan" "Northstar Mobility"', provider="synthetic", rank=1, requested_url="https://northstarmobility.example/team", final_url="https://northstarmobility.example/team", retrieval_status=RetrievalStatus.FULL_TEXT_RETRIEVED.value, full_text="Alex Morgan is a woman and member of the Northstar Mobility leadership team.", page_title="Northstar Mobility team", content_sha256="synthetic-1")],
        "SYN_002": [RetrievedSource(query='"Jordan Lee" "Helio Systems"', provider="synthetic", rank=1, requested_url="https://news.example/profile", final_url="https://news.example/profile", retrieval_status=RetrievalStatus.FULL_TEXT_RETRIEVED.value, full_text="Jordan Lee works with Helio Systems on international markets.", page_title="Synthetic professional profile", content_sha256="synthetic-2")],
        "SYN_003": [],
    }
    engine = AdmissibilityEngine(trusted_official_domains={"northstarmobility.example"})
    return pd.DataFrame([engine.evaluate(row, sources[row["person_id"]]).to_flat_dict() for row in rows])
...
if restricted:
    st.warning("RESTRICTED MODE is enabled. Uploaded names and organisations may be sent to the configured search provider. Use only in an approved private/local environment.")
else:
    st.info("PUBLIC DEMO MODE. Direct-identifier upload and live evidence retrieval are disabled. This deployment can safely demonstrate the method with synthetic data only.")
...
public_tab, restricted_tab = st.tabs(["Synthetic demonstration", "Restricted audit runner"])
with public_tab:
...
with restricted_tab:
    if not restricted:
        st.error("Restricted audit is disabled in public mode. Run locally/private with `PROFAIR_RESTRICTED_MODE=1`.")
    else:
        uploaded = st.file_uploader("Upload restricted audit CSV", type=["csv"])
        provider_name = st.selectbox("Search provider", ["SerpAPI", "Brave Search"])
        max_cases = st.number_input("Maximum cases for this run", min_value=1, max_value=1000, value=30, step=1)
        ...
        if st.button("Run restricted evidence audit", type="primary", disabled=run_disabled):
            ...
            with tempfile.TemporaryDirectory(prefix="profair_audit_") as temp:
                ...
                c3.metric("Man", int((results["final_category"] == "Man").sum()))
                c4.metric("Indeterminate / NC", int((results["A_i_B"] == 0).sum()))
                st.download_button("Download results CSV", data=results.to_csv(index=False).encode("utf-8-sig"), file_name="admissibility_results.csv", mime="text/csv")
                st.download_button("Download run manifest", data=json.dumps(manifest, ensure_ascii=False, indent=2).encode("utf-8"), file_name="run_manifest.json", mime="application/json")
                st.download_button("Download full audit bundle", data=_zip_directory(out_dir), file_name="profair_audit_output.zip", mime="application/zip")
